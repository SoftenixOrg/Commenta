class CommentsWidget {
  ...
  async deleteComment(commentId) {
    ...
    try {
      const response = await fetch(`${this.apiBase}/api/comments/${commentId}`, { method: "DELETE", credentials: "include" })
      const data = await response.json()
      if (data.status === "success") {
  ...
  updateCommentInList(commentId, updatedComment) {
    for (let i = 0; i < this.comments.length; i++) {
      if (this.comments[i].id === commentId) { this.comments[i] = updatedComment; return }
      if (this.comments[i].replies) {
        for (let j = 0; j < this.comments[i].replies.length; j++) {
          if (this.comments[i].replies[j].id === commentId) { this.comments[i].replies[j] = updatedComment; return }
        }
      }
    }
  }
  ...
  renderPagination() {
    ...
    if (!paginationContainer || this.totalPages <= 1) { paginationContainer.innerHTML = ""; return }
  ...
  showLoading() { document.getElementById("messages").innerHTML = `<div class="loading"><span class="loading-spinner"></span> Loading comments...</div>` }
  showError(message) { const msg = document.getElementById("messages"); if (msg) { msg.innerHTML = `<div class="error">${this.escapeHtml(message)}</div>`; setTimeout(() => { msg.innerHTML = "" }, 5000) } }
  showSuccess(message) { const msg = document.getElementById("messages"); if (msg) { msg.innerHTML = `<div class="success">${this.escapeHtml(message)}</div>`; setTimeout(() => { msg.innerHTML = "" }, 3000) } }
  escapeHtml(text) { const div = document.createElement("div"); div.textContent = text; return div.innerHTML }
  timeAgo(date) { const diff = Math.floor((new Date()-date)/1000); if(diff<60) return "just now"; if(diff<3600) return `${Math.floor(diff/60)}m ago`; if(diff<86400) return `${Math.floor(diff/3600)}h ago`; if(diff<2592000) return `${Math.floor(diff/86400)}d ago`; return date.toLocaleDateString() }
}

// Global instance
let commentsWidget
document.addEventListener("DOMContentLoaded", () => { if (document.getElementById("comments")) commentsWidget = new CommentsWidget() })
